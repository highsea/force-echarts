...
//绘图 标准折线图
function bLine (titleArr ,dataArr, categoryArr, seriesArr, mainId){
    ...
    xAxis : [
        {
            type : 'category',
            boundaryGap : false,
            data : dataArr/*['周一','周二','周三','周四','周五','周六','周日']*/
        }
    ],
    yAxis : [
        {
...
function pie_view(titleArr, categoryArr, seriesArr, mianId){
    ...
    option = {
        ...
        tooltip : {
            trigger: 'item',
            formatter: "{a} 【点击查看用户信息】<br/>{b} <br/> 数据: {c} (占{d}%)"
        },
        legend: {
            orient : 'vertical',
            x : 'left',
            ...
            for (var idx in selected) {
                ...
                for (var i = 0, l = serie.data.length; i < l; i++) {
                    if (selected[idx][i]) {
                        ...
                        //       '【UID' + i +'】' +serie.data[i].uid;
                        window.open('user.php?user='+ serie.data[i].name +':'+serie.data[i].uid+'&marketing='+marketing, serie.data[i].uid,'height=800,width=960,top=0,left=0,toolbar=yes,menubar=yes,scrollbars=yes,resizable=yes,location=yes,status=yes');
                    }
                }
            }
            //alert(str);
